...
function calculateQuote() {
    ...
    // Giá bảo hành = Tổng giá gốc + 10%
    const warrantyFee = totalOriginalPrice * 0.1;
    const warrantyPrice = totalOriginalPrice + warrantyFee;
    
    // Tính VAT (8%) - luôn áp dụng vì đã >= 200k
    ...
    if (breakdown) {
        ...
        breakdown.innerHTML = `
            ...
            <div class="breakdown-item total-before-vat">
                <span class="breakdown-label">Tổng trước VAT:</span>
                <span class="breakdown-value">${formatPrice(warrantyPrice)}đ</span>
            </div>
            <div class="breakdown-item">
                <span class="breakdown-label">VAT (8%):</span>
...
• Tổng trước VAT: ${formatPrice(warrantyPrice)}đ
• VAT (8%): ${formatPrice(vatAmount)}đ
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 100px;">Thời hạn (Tháng)</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>Gói dịch vụ Nâng cấp ${item.product.name}</td>
                    <td class="text-right">${formatPrice(item.product.price)}</td>
                    <td class="text-center">${item.quantity}</td>
                    <td class="text-center">${item.product.duration < 1 ? '03' : String(item.product.duration).padStart(2, '0')}</td>
                    <td class="text-right">${formatPrice(item.product.price * item.quantity)}</td>
                </tr>
                `).join('')}
                <tr>
                    <td class="text-center">${products.length + 1}</td>
                    <td>Dịch vụ bảo hành</td>
                    <td class="text-right">${formatPrice(warrantyFee)}</td>
                    <td class="text-center">1</td>
                    <td class="text-center">03</td>
                    <td class="text-right">${formatPrice(warrantyFee)}</td>
                </tr>
                <tr class="subtotal-row">
                    <td colspan="5" class="text-right"><strong>Tạm tính (chưa VAT)</strong></td>
                    <td class="text-right"><strong>${formatPrice(warrantyPrice)}</strong></td>
                </tr>
                <tr class="vat-row">
                    <td colspan="5" class="text-right"><strong>Thuế VAT (8%):</strong></td>
                    <td class="text-right"><strong>${formatPrice(vatAmount)}</strong></td>
                </tr>
                <tr class="total-row">
                    <td colspan="5" class="text-right"><strong>Tổng tiền đã bao gồm thuế VAT</strong></td>
                    <td class="text-right"><strong>${formatPrice(finalPrice)}</strong></td>
                </tr>
                <tr class="note-row">
                    <td colspan="6">
                        <strong>Viết bằng chữ:</strong> ${convertNumberToWords(finalPrice)} đồng${shouldApplyVAT ? ' (Đã bao gồm VAT)' : ' (Chưa bao gồm VAT)'}.
                    </td>
                </tr>
            </tbody>
        </table>
